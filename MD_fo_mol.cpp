...
cout << " " << endl; 
cout << "２行目　　全粒子数　全電価数  相互作用指定" << endl; 
cout << "　　　　　全粒子数 > 全電価数。" << endl; 
cout << "　　　　　相互作用: default=C: Coulomb,現在はCoulombのみ有効 " << endl; 
cout << " " << endl; 
cout << "３行目　　シミュレーションの試行回数" << endl; 
cout << " " << endl; 
cout << "４行目以降に初期原子データを書く " << endl; 
cout << "書式 " << endl; 
cout << "ラベル  質量[amu]  x[nm] y[nm] z[nm] vx[nm/ps] vy[nm/ps] vz[nm/ps] 初期電荷 最大電荷" << endl; 
...
for(i=0;i<sim_data.trial;i++) {
	sim_data.get_data(argv[1]);	//初期データ取得
